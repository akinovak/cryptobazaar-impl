...
mod tests {
    ...
    //     // chs = 10, 20
    //     let chs: Vec<_> = (0..2).map(|i| F::from((i + 1) * 10)).collect();

    //     let mut folded_lb = lb.clone();
    //     for i in 0..2 {
    //         folded_lb = fold_array_of_polys(&folded_lb, chs[i]);
    //     }
    ...
    //        folded_lb = l0 + a1l1 + a0l2 + a0a1l3
    //     */

    //     assert_eq!(folded_lb.len(), 1);
    //     let folded_lb_evals = vec![F::one(), chs[1], chs[0], chs[0] * chs[1]];
    //     let folded_lb_poly =
    //         DensePolynomial::from_coefficients_slice(&domain.ifft(&folded_lb_evals));
    //     assert_eq!(folded_lb_poly, folded_lb[0]);

    //     let folded_poly = build_folding_polynomial_slow(&chs);
    //     // 1 + 0x + a1x^2 + 0x^3 + a0x^4 + 0x^5 + a0a1x^6
    //     assert_eq!(folded_poly[0], F::one());
    //     assert_eq!(folded_poly[2], chs[1]);
    //     assert_eq!(folded_poly[4], chs[0]);
    //     assert_eq!(folded_poly[6], chs[0] * chs[1]);

    //     println!("{}", folded_poly.degree());

    //     let cfs = vec![F::one(), chs[1], chs[0], chs[0] * chs[1]];
    ...
    //     let y_4 = y_2 * y_2;
    //     let y_6 = y_4 * y_2;
    //     let pows = [F::one(), y_2, y_4, y_6];

    //     let f_eval = folded_poly.evaluate(&y);
    //     let fold_eval = evaluate_folding_polynomial(&chs, y);
    //     let f_eval_manual: F = cfs.iter().zip(pows.iter()).map(|(&ci, &pi)| ci * pi).sum();
    //     assert_eq!(f_eval, f_eval_manual);

    //     // let folded_lb = fold_array_of_polys(&lb, F::from(100));
    // }
}
